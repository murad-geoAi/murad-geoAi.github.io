...
size. The RNG seed is pinned, so reruns should reproduce the same figures — a
...
On first run it downloads Leaflet 1.9.4 into `assets/vendor/leaflet/` (skipped
if already present). Those files are committed so the demo makes no CDN
requests.
